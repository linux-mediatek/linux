...
static int mt6789_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
...
static int mt6789_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	ret = regmap_attach_dev(dev, afe->regmap, &mt6789_afe_regmap_config);
	if (ret)
...
static const struct dev_pm_ops mt6789_afe_pm_ops = {
	RUNTIME_PM_OPS(mt6789_afe_runtime_suspend,
