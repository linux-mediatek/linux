...
static irqreturn_t sc8541_irq_handler(int irq, void *data)
{
	...
	u8 flag = 0;
	sc8541_read_block(sc, 0x1a, &flag, 1);
	if (flag & BIT(2)) {
		dev_err(sc->dev, "vbus present!");
		sc8541_field_write(sc, CHG_EN, 1);
	}
	sc8541_read_block(sc, 0x1c, &flag, 1);
	if (flag & BIT(6)) {
		dev_err(sc->dev, "switch charging!!");
	}
...
static int sc8541_register_interrupt(struct sc8541_chip *sc)
{
    ...
    if (gpio_is_valid(sc->irq_gpio)) {
        ret = gpio_request_one(sc->irq_gpio, GPIOD_IN,"sc8541_irq");
        if (ret) {
