...
struct screen_info screen_info_fb;
static int __init simplefb_earlycon_remap_fb(void)
{
	/* bail if there is no bootconsole or it has been disabled already */
	if (!earlycon_console || !(earlycon_console->flags & CON_ENABLED))
		return 0;
	simplefb_fb = memremap(fb_base, screen_info_fb.lfb_size, MEMREMAP_WB);
...
static void simplefb_earlycon_clear_scanline(unsigned int y)
{
	unsigned long *dst;
	u16 len;
	len = screen_info_fb.lfb_linelength;
...
static void simplefb_earlycon_scroll_up(void)
{
	...
	u32 i, height;
	len = screen_info_fb.lfb_linelength;
	height = screen_info_fb.lfb_height;
